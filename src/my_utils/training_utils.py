...
def parse_args(input_args=None):
    ...
    # For mixture of LoRA experts pixel training
    parser.add_argument("--num_experts_pix", default=1, type=int, help="Number of routed LoRA experts for pixel branch. If set to 1, it is equivalent to single LoRA training.")
    parser.add_argument("--top_k_pix", default=1, type=int, help="Top-k experts to use during training for pixel branch.")
    parser.add_argument("--num_shared_experts_pix", default=0, type=int, help="Number of shared experts (always activated, not routed) for pixel branch.")

    # Load balancing loss for MoLE
    parser.add_argument("--use_load_balance_loss", action="store_true",
        help="Enable load balancing auxiliary loss for MoLE training")
    parser.add_argument("--lambda_load_balance", default=0.01, type=float,
        help="Weight for load balancing auxiliary loss")

    # Frequency-based routing for MoLE
    parser.add_argument("--use_freq_gate", action="store_true",
        help="Enable frequency-based routing. Uses degradation features from frequency domain to guide expert selection.")
    parser.add_argument("--freq_dim", default=7, type=int,
        help="Dimension of frequency features. Default is 7 (4 frequency bands + blur/jpeg/noise scores)")
    parser.add_argument("--num_freq_bands", default=4, type=int,
        help="Number of radial frequency bands to extract from input image")

    # Orthogonal Gradient Projection (GPM-SVD)
    parser.add_argument("--ortho_enabled", action="store_true", default=False,
        help="Enable orthogonal gradient projection for Semantic LoRA training")
    parser.add_argument("--svd_energy_threshold", type=float, default=0.95,
        help="Energy threshold for SVD truncation (default: 0.95)")
    parser.add_argument("--save_svd_subspaces", action="store_true", default=False,
        help="Save extracted SVD subspaces to checkpoint directory")
    parser.add_argument("--lambda_l2_sem", type=float, default=None,
        help="Separate L2 weight for semantic phase (default: None = use lambda_l2)")
    parser.add_argument("--resume_training_state", default=None, type=str,
        help="Path to training state (.pt) for optimizer/scheduler/step resume")
