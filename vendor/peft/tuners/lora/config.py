...
class LoraConfig(PeftConfig):
    ...
    # For mixture of lora
    number_of_lora: int = field(
        default=1,
        metadata={
            "help": "Number of different lora adapters to use in mixture of lora experts (routed experts)."
        }
    )
    top_k_lora: int = field(
        default=1,
        metadata={
            "help": "Number of top-k experts to use during forward pass. If equal to number_of_lora, all experts are used."
        }
    )
    num_shared_experts: int = field(
        default=0,
        metadata={
            "help": "Number of shared experts that are always activated (not routed). Output is summed with routed experts."
        }
    )
    use_freq_gate: bool = field(
        default=False,
        metadata={
            "help": "Enable frequency-based routing. When True, uses FrequencyModulatedGate that incorporates "
                    "frequency domain features (degradation hints) to help expert selection."
        }
    )
    freq_dim: int = field(
        default=7,
        metadata={
            "help": "Dimension of frequency features for frequency-based routing. Default is 7 "
                    "(4 frequency bands + blur score + JPEG score + noise score)."
        }
    )
